...
df_full <- df_full %>% 
  mutate(early_extub = case_when(re_intub == "Yes" ~ "No",
                          TRUE ~ early_extub))
...
df_full <- df_full %>% 
  mutate(extub_days = case_when(early_extub == "Yes" ~ 0,
                          TRUE ~ extub_d_day))
...
df_full <- df_full %>% 
  mutate(cpb_time = case_when(bypass == "No" ~ 0,
                          TRUE ~ cpb_t))


# Select variables for final data set
df_final <- df_full %>%
  dplyr::select(blind_id, blind_site_id, sex, race, ethnicity, diag, chrom_anomaly, proc_age, op_w, cpb_time, cc_time, early_extub, extub_days, enteral_t_oral_initiate, oral_postextub)
```
